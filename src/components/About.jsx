...
const ServiceCard = ({ index, title, icon }) => (
	<Tilt className="xs:w-[250px] w-full">
		<motion.div variants={fadeIn("right", "spring", index * 0.5, 0.75)} className="w-full green-pink-gradient p-[1px] rounded-[20px] shadow-card">
			...
			>
				<img src={icon} alt="web-development" className="w-48 h-48 object-cover" loading="lazy" />
	...
	return (
		<>
			<motion.div variants={textVariant()}>
				<p className={styles.sectionSubText}>{section2.subtitle}</p>
				<h2 className={styles.sectionHeadText}>{section2.title}</h2>
			</motion.div>
